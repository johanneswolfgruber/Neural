...
std::vector<int> Audio16IO::getDrivers() {
...
std::string Audio16IO::getDriverName(int driver) {
	switch (driver) {
...
void Audio16IO::setDriver(int driver) {
	// Close device
	setDevice(-1, 0);
...
bool Audio16IO::getDeviceInfo(int device, RtAudio::DeviceInfo *deviceInfo) {
	if (!deviceInfo)
		return false;
...
int Audio16IO::getDeviceChannels(int device) {
	if (device < 0)
		return 0;
...
std::string Audio16IO::getDeviceName(int device) {
	if (device < 0)
		return "";
...
std::string Audio16IO::getDeviceDetail(int device, int offset) {
	if (device < 0)
		return "";
...
void Audio16IO::setDevice(int device, int offset) {
	closeStream();
	this->device = device;
	this->offset = offset;
	openStream();
...
std::vector<int> Audio16IO::getSampleRates() {
	if (rtAudio) {
		try {
...
void Audio16IO::setSampleRate(int sampleRate) {
...
std::vector<int> Audio16IO::getBlockSizes() {
	if (rtAudio) {
...
void Audio16IO::setBlockSize(int blockSize) {
...
void Audio16IO::setChannels(int numOutputs, int numInputs) {
...
static int rtCallback(void *outputBuffer, void *inputBuffer, unsigned int nFrames, double streamTime, RtAudioStreamStatus status, void *userData) {
	Audio16IO *audio16IO = (Audio16IO*) userData;
	assert(audio16IO);
	audio16IO->processStream((const float *) inputBuffer, (float *) outputBuffer, nFrames);
	return 0;
}

void Audio16IO::openStream() {
	if (device < 0)
		return;
	...
	if (rtAudio) {
		...
		bridgeAudioSubscribe(device, (AudioIO*)this);
	}
}

void Audio16IO::closeStream() {
	setChannels(0, 0);
	...
	if (rtAudio) {
		...
		bridgeAudioUnsubscribe(device, (AudioIO*)this);
	}

	onCloseStream();
}

json_t *Audio16IO::toJson() {
...
void Audio16IO::fromJson(json_t *rootJ) {
	closeStream();
